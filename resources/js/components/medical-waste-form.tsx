...
import { MedicalWaste } from "@/types";
...
import { destroy, store, update } from "@/routes/medicalWaste";
...
export const MedicalWasteForm = ({ item, id }: MedicalWasteFormProps) => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const commonOptions = {
            ...
            onSuccess: () => {
                ...
                setOpen(false);
                router.reload();
                toast.success(`Medical waste data has been ${item ? 'updated' : 'saved'} successfully.`);
            },
            onError: () => {
                toast.error(`Failed to ${item ? 'update' : 'save'} medical waste data.`);
            },
        };
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Medical Waste Form</DialogTitle>
                    <DialogDescription>
                        {item ? 'Update medical waste information' : 'Fill out the form to add a new medical waste'}
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <DialogFooter className="gap-2 pt-4">
                            <div className="flex justify-between w-full">
                                    ...
                                    {item && (
                                        <AlertDialog>
                                            <AlertDialogTrigger asChild disabled={processing}>
                                                {Can('medicalWasteData.delete') && <Button type="button" variant="destructive" className="hover:!bg-red-900 cursor-pointer">Delete</Button>}
                                            </AlertDialogTrigger>
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
                                                    <AlertDialogDescription>This action cannot be undone. This will permanently delete the medical waste data from the server.</AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {Can('medicalWasteData.edit') && <Button type="submit" disabled={processing} className="cursor-pointer">
                                        {item ? 'Update' : 'Save'}
                                    </Button>}
                                </div>
                            </div>
