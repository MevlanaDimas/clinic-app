...
import { index } from '@/routes/marketings';
import { Marketing, type BreadcrumbItem } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Marketing Cost Data',
        href: index().url,
    },
];

export default function MarketingsIndex() {
    const { marketings }: any = usePage().props;
    const meta = marketings.meta;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Marketing Cost Data" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <form onSubmit={searchData} className="w-full md:w-1/3">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                id="search"
                                name="search"
                                placeholder="Search marketing costs..."
                    ...
                    {Can('marketingData.create') && <MarketingForm />}
                </div>
                    ...
                    <Table>
                        <TableHeader className='border-b bg-muted'>
                            <TableRow>
                                ...
                                <TableHead className='text-center'>Marketing ID</TableHead>
                        ...
                        <TableBody>
                            {marketings.data.length > 0 ? (
                                marketings.data.map((item: Marketing, index: number) => (
                                    <TableRow key={item.id} className="transition-colors hover:bg-muted/50">
                                        <TableCell className="text-center">{meta ? meta.from + index : index + 1}</TableCell>
                                        <TableCell className='text-center'>{item.marketing_id}</TableCell>
                                        ...
                                        <TableCell className="flex flex-row items-center justify-center gap-5 text-center">
                                            <MarketingForm item={item} id={item.id} />
                                        </TableCell>
                                    </TableRow>
                                ))) : (
                                    <TableRow>
