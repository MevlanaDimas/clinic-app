...
import { MedicalWasteForm } from '@/components/medical-waste-form';
...
import { index } from '@/routes/medicalWaste';
import { MedicalWaste, type BreadcrumbItem } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Medical Waste Cost Data',
        href: index().url,
    },
];

export default function MedicalWasteIndex() {
    const { medicalWastes }: any = usePage().props;
    const meta = medicalWastes.meta;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Medical Waste Cost Data" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <form onSubmit={searchData} className="w-full md:w-1/3">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                id="search"
                                name="search"
                                placeholder="Search medical waste costs..."
                    ...
                    {Can('medicalWasteData.create') && <MedicalWasteForm />}
                </div>
                    ...
                    <Table>
                        <TableHeader className='border-b bg-muted'>
                            <TableRow>
                                ...
                                <TableHead className='text-center'>Medical Waste ID</TableHead>
                        ...
                        <TableBody>
                            {medicalWastes.data.length > 0 ? (
                                medicalWastes.data.map((item: MedicalWaste, index: number) => (
                                    <TableRow key={item.id} className="transition-colors hover:bg-muted/50">
                                        <TableCell className="text-center">{meta ? meta.from + index : index + 1}</TableCell>
                                        <TableCell className='text-center'>{item.medical_waste_id}</TableCell>
                                        ...
                                        <TableCell className="flex flex-row items-center justify-center gap-5 text-center">
                                            <MedicalWasteForm item={item} id={item.id} />
                                        </TableCell>
                                    </TableRow>
                                ))) : (
                                    <TableRow>
