...
import { StaffSalaryForm } from '@/components/staff-salary-form';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import AppLayout from '@/layouts/app-layout';
import { handlePerPageChange } from '@/lib/utils';
import { index } from '@/routes/staffSalary';
import { StaffSalary, type BreadcrumbItem } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Staff Salaries Data',
        href: index().url
    },
];

export default function StaffSalariesIndex() {
    const { staffSalaries }: any = usePage().props;
    const meta = staffSalaries.meta;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Staff Salaries Data" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    {Can('staffSalaryData.create') && <StaffSalaryForm />}
                </div>
                    ...
                    <Table>
                        <TableHeader className='border-b bg-muted'>
                            <TableRow>
                                ...
                                <TableHead className='text-center'>Monthly Salary</TableHead>
                                <TableHead className='text-center'>Updated At</TableHead>
                                <TableHead className='text-center'>Action</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {staffSalaries.data.length > 0 ? (
                                staffSalaries.data.map((staffSalary: StaffSalary, index: number) => (
                                    <TableRow key={staffSalary.id} className="transition-colors hover:bg-muted/50">
                                        <TableCell className="text-center">{meta ? meta.from + index : index + 1}</TableCell>
                                        <TableCell className="text-center">{staffSalary.id}</TableCell>
                                        <TableCell className="text-center">{staffSalary.name}</TableCell>
                                        <TableCell className="text-center">{staffSalary.position}</TableCell>
                                        <TableCell className='text-center'>{staffSalary.staff_in_user.email}</TableCell>
                                        <TableCell className="text-center">{formatCurrency(staffSalary.staff_monthly_salary)}</TableCell>
                                        <TableCell className="text-center">{staffSalary.updated_at}</TableCell>
                                        <TableCell className="items-center justify-center text-center">
                                            <StaffSalaryForm staffSalary={staffSalary} id={staffSalary.id} />
                                        </TableCell>
                                    </TableRow>
                                ))) : (
                                    <TableRow>
